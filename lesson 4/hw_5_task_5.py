...
patterns = [r"^.{120,170}\W{4}",
            r"It.{169}",
            r"The.{50,120}ies\W{4}",
            r"\W{3} The.{60,}.ft\W",
            r"\W{1}A .{30,}.sen\W",
            r"\W{1} The A.{50,}.in\W{2}"]

separated_sentences = []

for pattern in patterns:
    result_of_search = re.findall(pattern, text)
    to_write = f'{str(result_of_search[0]).replace(".", " ").strip()}.'
    separated_sentences.append(to_write)
